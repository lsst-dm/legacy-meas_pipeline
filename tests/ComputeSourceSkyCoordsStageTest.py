...
class ComputeSourceSkyCoordsStageTestCase(unittest.TestCase):
    ...
    def testStage(self):
        ...
        for s in sources:
            self.assertNotEqual(s.getRaFlux().asRadians(), 0.0)
            self.assertNotEqual(s.getRaFluxErr().asRadians(), 0.0)
            self.assertNotEqual(s.getDecFlux().asRadians(), 0.0)
            self.assertNotEqual(s.getDecFluxErr().asRadians(), 0.0)
            self.assertNotEqual(s.getRaAstrom().asRadians(), 0.0)
            self.assertNotEqual(s.getRaAstromErr().asRadians(), 0.0)
            self.assertNotEqual(s.getDecAstrom().asRadians(), 0.0)
            self.assertNotEqual(s.getDecAstromErr().asRadians(), 0.0)
            self.assertNotEqual(s.getRaPeak().asRadians(), 0.0)
            self.assertNotEqual(s.getDecPeak().asRadians(), 0.0)
