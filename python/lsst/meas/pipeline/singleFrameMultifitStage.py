...
import lsst.meas.utils.multifit as multifitUtils

class SingleFrameMultifitStageParallel(harnessStage.ParallelProcessing):
    def setup(self):
        self.log = Log(self.log, "SingleFrameMultifitStage - parallel")

        policyFile = pexPolicy.DefaultPolicyFile("meas_pipeline", 
            "SingleFrameMultifitStageDictionary.paf", "policy")
    ...
    def process(self, clipboard):
        ...
        if(not sourceSet or not fpSet or not exposure or not psf):
            self.log.log(Log.FATAL, "Missing necessary inputs")
